...
public class HomeCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 1) {
            try {
                ...
                if (home >= 1 && home <= Homes.getInstance().getConfig().getInt("Settings.MaxHomes", 5)) {
                    ...
                    player.sendMessage(MessageUtil.getPrefix() + MessageUtil.get("Messages.NotYet"));
                    return false;
                }
        ...
        if (player.hasPermission("home.admin") && (args.length == 2 || args.length == 3)) {
            ...
            if (target == null) {
                player.sendMessage(MessageUtil.getPrefix() + "§cPlayer was not found!");
                return false;
            }
        ...
        player.sendMessage(" ");
        player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home");
        player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home <1-" + Homes.getInstance().getAmount(player) + ">");
        player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home set <1-" + Homes.getInstance().getAmount(player) + ">");
        player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home delete <1-" + Homes.getInstance().getAmount(player) + ">");
        player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home remove <1-" + Homes.getInstance().getAmount(player) + ">");
        if (player.hasPermission("home.admin")) {
            player.sendMessage(" ");
            player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home <player> <1-" + Homes.getInstance().getConfig().getInt("Settings.MaxHomes", 5) + ">");
            player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home <player> set <1-" + Homes.getInstance().getConfig().getInt("Settings.MaxHomes", 5) + ">");
            player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home <player> delete <1-" + Homes.getInstance().getConfig().getInt("Settings.MaxHomes", 5) + ">");
            player.sendMessage(MessageUtil.getPrefix() + "Usage: §c/home <player> remove <1-" + Homes.getInstance().getConfig().getInt("Settings.MaxHomes", 5) + ">");
        }
        player.sendMessage(" ");
        return false;
    }
